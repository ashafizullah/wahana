...
- Several WhatsApp Web accounts, each with its own isolated login — show any number of them side by side in one split view, drag the dividers to give one more room (double-click to even them out), reorder, hide, rename / remove at will; when the window is too narrow the panes wrap onto extra rows
