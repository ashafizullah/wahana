...
function label(m: WAMessage, resolve: MentionResolver) {
...
export async function exportChat(chatName: string, messages: WAMessage[], format: ExportFormat, resolve: MentionResolver) {
  ...
  let out: string;
  if (format === "json") {
    out = JSON.stringify(sorted.map((m) => ({ id: m.id, timestamp: m.timestamp, date: new Date(m.timestamp * 1000).toISOString(), from: label(m, resolve), fromMe: m.fromMe, body: m.body, media: m.hasMedia ? m.media?.mimetype : undefined, kind: kind(m) || undefined })), null, 2);
  } else if (format === "txt") {
    out = sorted.map((m) => `[${new Date(m.timestamp * 1000).toLocaleString()}] ${label(m, resolve)}: ${[kind(m), m.body].filter(Boolean).join(" ")}`).join("\n");
  } else {
    out = `<!doctype html><meta charset="utf-8"><title>${esc(chatName)}</title><style>body{font-family:system-ui;max-width:720px;margin:2rem auto;background:#efeae2;padding:0 1rem}.m{max-width:75%;margin:.35rem 0;padding:.4rem .7rem;border-radius:10px;background:#fff;white-space:pre-wrap;word-break:break-word}.me{margin-left:auto;background:#d9fdd3}.h{font-size:.7rem;color:#555}.d{text-align:center;font-size:.75rem;color:#666;margin:1rem 0}</style><h2>${esc(chatName)}</h2>` +
      sorted.map((m, i, a) => {
        ...
        return `${div}<div class="m${m.fromMe ? " me" : ""}"><div class="h">${esc(label(m, resolve))} · ${new Date(m.timestamp * 1000).toLocaleTimeString()}</div>${esc([kind(m), stripWaMarkdown(m.body || "")].filter(Boolean).join(" "))}</div>`;
      }).join("");
  }
