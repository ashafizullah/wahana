...
export function transcript(messages: WAMessage[], resolve: MentionResolver) {
  return [...messages]
    ...
    .map(
      (m) =>
        `[${new Date(m.timestamp * 1000).toLocaleString()}] ${messageLabel(m, resolve)}: ${[messageKind(m), m.body].filter(Boolean).join(" ")}`,
    )
    .join("\n");
}
  ...
  if (format === "json") {
    out = JSON.stringify(
      sorted.map((m) => ({
        ...
        from: messageLabel(m, resolve),
        fromMe: m.fromMe,
        body: m.body,
        media: m.hasMedia ? m.media?.mimetype : undefined,
        kind: messageKind(m) || undefined,
      })),
      null,
      2,
    );
