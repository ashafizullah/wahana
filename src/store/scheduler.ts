...
const COLS = "id, profile, session, target_type, target_id, target_name, kind, text, media_mime, media_name, next_run, anchor, repeat, weekdays, enabled, created_at, last_run, last_status, last_error, runs";
...
export async function upsertSchedule(s: Omit<Schedule, "created_at" | "last_run" | "last_status" | "last_error" | "runs" | "anchor"> & { created_at?: number; anchor?: number | null }) {
  const d = await db();
  await d.execute(
    ...
    [s.id, s.profile, s.session, s.target_type, s.target_id, s.target_name, s.kind, s.text, s.media_b64, s.media_mime, s.media_name, s.next_run, s.anchor ?? s.next_run, s.repeat, s.weekdays, s.enabled, s.created_at ?? Math.floor(Date.now() / 1000)],
  );
}
...
export function nextOccurrence(s: Pick<Schedule, "next_run" | "repeat" | "weekdays"> & { anchor?: number | null }, after: number): number | null {
