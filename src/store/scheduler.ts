...
const COLS = "id, profile, session, target_type, target_id, target_name, kind, text, media_mime, media_name, next_run, repeat, weekdays, enabled, created_at, last_run, last_status, last_error, runs";

export async function listSchedules(profile: string): Promise<Schedule[]> {
  const d = await db();
  return d.select<Schedule[]>(`SELECT ${COLS}, NULL AS media_b64 FROM schedules WHERE profile = $1 ORDER BY enabled DESC, next_run ASC`, [profile]);
}
...
export async function upsertSchedule(s: Omit<Schedule, "created_at" | "last_run" | "last_status" | "last_error" | "runs"> & { created_at?: number }) {
  const d = await db();
  await d.execute(
    `INSERT INTO schedules (id, profile, session, target_type, target_id, target_name, kind, text, media_b64, media_mime, media_name, next_run, repeat, weekdays, enabled, created_at)
     ...
     ON CONFLICT(id) DO UPDATE SET session=excluded.session, target_type=excluded.target_type, target_id=excluded.target_id, target_name=excluded.target_name,
       ...
       media_name=COALESCE(excluded.media_name, schedules.media_name), next_run=excluded.next_run, repeat=excluded.repeat, weekdays=excluded.weekdays, enabled=excluded.enabled`,
    [s.id, s.profile, s.session, s.target_type, s.target_id, s.target_name, s.kind, s.text, s.media_b64, s.media_mime, s.media_name, s.next_run, s.repeat, s.weekdays, s.enabled, s.created_at ?? Math.floor(Date.now() / 1000)],
  );
}
...
export async function recordRun(s: Schedule, status: Run["status"], opts: { error?: string; messageId?: string; nextRun: number | null }) {
  const d = await db();
  const now = Math.floor(Date.now() / 1000);
  await d.execute("INSERT INTO schedule_runs (schedule_id, ran_at, status, error, message_id) VALUES ($1,$2,$3,$4,$5)", [s.id, now, status, opts.error ?? null, opts.messageId ?? null]);
  await d.execute(
    "UPDATE schedules SET last_run = $1, last_status = $2, last_error = $3, runs = runs + 1, next_run = COALESCE($4, next_run), enabled = CASE WHEN $4 IS NULL THEN 0 ELSE enabled END WHERE id = $5",
    [now, status, opts.error ?? null, opts.nextRun, s.id],
  );
}
...
export function nextOccurrence(s: Pick<Schedule, "next_run" | "repeat" | "weekdays">, after: number): number | null {
  if (s.repeat === "once") return null;
  const base = new Date(s.next_run * 1000);
  const hh = base.getHours(), mm = base.getMinutes();
  ...
  if (s.repeat === "weekly") {
    const days = (s.weekdays ?? String(base.getDay())).split(",").map(Number).filter((n) => n >= 0 && n <= 6);
    for (let i = 0; i < 8; i++) {
