...
import { CalendarClock, Plus, Trash2, Pencil, Loader2, X, History, Play, Pause, AlertTriangle, CheckCircle2, Clock, Megaphone, Users, User, CircleDashed, Paperclip } from "lucide-react";
    ...
    <div className="flex-1 min-w-0 flex flex-col">
      <div className="h-14 shrink-0 flex items-center gap-3 px-6 border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
        ...
        <div>
          ...
          <p className="text-[11px] text-neutral-500">Sent by this app while it is running (tray is fine). Jobs more than {GRACE_SECONDS / 60} min late are marked missed.</p>
        </div>
        <Button className="ml-auto" onClick={() => setEditing("new")}><Plus size={14} /> New schedule</Button>
      </div>
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {!q.isLoading && list.length === 0 && (
          <div className="text-sm text-neutral-500">No schedules yet. Create one to send a message, story or channel post at a set time — once or on a repeat.</div>
        )}
...
function Group({ title, items, onEdit, onHistory }: { title: string; items: Schedule[]; onEdit: (s: Schedule) => void; onHistory: (s: Schedule) => void }) {
  const qc = useQueryClient();
  return (
    <section className="space-y-2">
      ...
      <ul className="space-y-2">
        {items.map((s) => (
          <li key={s.id} className="flex items-center gap-3 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-4 py-3">
            <div className={cn("w-9 h-9 rounded-full grid place-items-center shrink-0", s.enabled ? "bg-wa-dark/10 text-wa-dark" : "bg-neutral-100 dark:bg-neutral-800 text-neutral-400")}>
              <TargetIcon s={s} />
            </div>
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[11px] mt-0.5 flex items-center gap-2">
                <span className="flex items-center gap-1 text-neutral-600 dark:text-neutral-300"><Clock size={11} /> {s.enabled ? `Next ${fmt(s.next_run)}` : s.last_run ? `Last ${fmt(s.last_run)}` : fmt(s.next_run)}</span>
                {s.last_status === "ok" && <span className="flex items-center gap-1 text-emerald-600"><CheckCircle2 size={11} /> sent {s.runs}×</span>}
                {s.last_status === "error" && <span className="flex items-center gap-1 text-red-600" title={s.last_error ?? ""}><AlertTriangle size={11} /> failed</span>}
                {s.last_status === "missed" && <span className="flex items-center gap-1 text-amber-600" title={s.last_error ?? ""}><AlertTriangle size={11} /> missed</span>}
              </div>
            </div>
            <Button size="sm" variant="ghost" title="History" onClick={() => onHistory(s)}><History size={14} /></Button>
            <Button size="sm" variant="ghost" title={s.enabled ? "Pause" : "Resume"} onClick={async () => { await setEnabled(s.id, !s.enabled); qc.invalidateQueries({ queryKey: ["schedules"] }); }}>
              {s.enabled ? <Pause size={14} /> : <Play size={14} />}
            </Button>
            <Button size="sm" variant="ghost" title="Edit" onClick={async () => onEdit((await getSchedule(s.id)) ?? s)}><Pencil size={14} /></Button>
            <Button size="sm" variant="ghost" className="text-red-600" title="Delete" onClick={async () => { if (await confirm({ title: "Delete this schedule?", danger: true, confirmLabel: "Confirm" })) { await deleteSchedule(s.id); qc.invalidateQueries({ queryKey: ["schedules"] }); } }}><Trash2 size={14} /></Button>
          </li>
        ))}
      </ul>
    </section>
...
function ScheduleForm({ session, profile, initial, onClose, onSaved }: { session: string; profile: string; initial: Schedule | null; onClose: () => void; onSaved: () => void }) {
  ...
  const kind: Kind = file ? (file.type.startsWith("image/") ? "image" : file.type.startsWith("video/") ? "video" : "file") : keepMedia && initial ? initial.kind : "text";
  ...
  const save = async () => {
    ...
    try {
      ...
      if (repeat !== "once" && firstRun <= now) firstRun = nextOccurrence({ next_run: whenUnix, repeat, weekdays: weekdays.join(",") }, now) ?? whenUnix;
    ...
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="w-[560px] max-h-[88vh] flex flex-col rounded-xl bg-white dark:bg-neutral-900 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-1 mb-2">
              <Button size="sm" variant={targetType === "chat" ? "primary" : "secondary"} onClick={() => setTargetType("chat")}>Contact / group / channel</Button>
              <Button size="sm" variant={targetType === "status" ? "primary" : "secondary"} onClick={() => setTargetType("status")}><CircleDashed size={12} /> My status</Button>
            </div>
            {targetType === "chat" && (
              <div className="rounded-lg border border-neutral-200 dark:border-neutral-700">
                {targetId ? (
                  <div className="flex items-center gap-2 px-3 py-2">
                    ...
                    <span className="flex-1 truncate text-sm">{targetName || displayId(targetId)} <span className="text-xs text-neutral-500">{targetId}</span></span>
                    <button onClick={() => { setTargetId(""); setTargetName(""); }}><X size={14} /></button>
                  </div>
                ) : (
                  <>
                    <Input className="border-0 rounded-b-none" placeholder="Search chats, groups, channels…" value={q} onChange={(e) => setQ(e.target.value)} autoFocus />
                    <div className="max-h-40 overflow-y-auto border-t border-neutral-100 dark:border-neutral-800">
                      {candidates.map((c) => (
                        <button key={c.id} onClick={() => { setTargetId(c.id); setTargetName(c.name ?? ""); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-neutral-100 dark:hover:bg-neutral-800">
                      ...
                      {q.trim() && /^\+?\d{8,}$/.test(q.trim()) && (
                        <button onClick={() => { const d = q.replace(/\D/g, ""); setTargetId(`${d}@c.us`); setTargetName(""); }} className="w-full px-3 py-1.5 text-left text-sm text-wa-dark hover:bg-neutral-100 dark:hover:bg-neutral-800">
          ...
          <div>
            ...
            <textarea value={text} onChange={(e) => setText(e.target.value)} rows={4} placeholder="Supports WhatsApp formatting: *bold* _italic_ ~strike~" className="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-transparent px-3 py-2 text-sm outline-none" />
            <div className="flex items-center gap-2 mt-1.5">
              <input ref={fileRef} type="file" hidden accept={targetType === "status" ? "image/*,video/*" : undefined} onChange={(e) => { setFile(e.target.files?.[0] ?? null); setKeepMedia(false); }} />
              <Button size="sm" variant="secondary" onClick={() => fileRef.current?.click()}><Paperclip size={12} /> {file || keepMedia ? "Change attachment" : "Attach photo / video / file"}</Button>
              {(file || keepMedia) && (
                <span className="text-xs text-neutral-500 flex items-center gap-1">
                  {file?.name ?? initial?.media_name} <button onClick={() => { setFile(null); setKeepMedia(false); }}><X size={12} /></button>
                </span>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="datetime-local" value={when} min={toLocalInput(nowUnix)} onChange={(e) => setWhen(e.target.value)} className={cn(repeat === "once" && inPast && "border-red-500")} />
              {repeat === "once" && inPast && <div className="text-[11px] text-red-600 mt-1">This time has already passed.</div>}
            </div>
            <div>
              <Label>Repeat</Label>
              <select value={repeat} onChange={(e) => setRepeat(e.target.value as Repeat)} className="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm outline-none">
          ...
          {repeat === "weekly" && (
            <div className="flex gap-1">
              {DAYS.map((d, i) => (
                <button key={d} onClick={() => setWeekdays((w) => (w.includes(i) ? w.filter((x) => x !== i) : [...w, i]))} className={cn("flex-1 rounded-lg py-1 text-xs font-medium", weekdays.includes(i) ? "bg-wa-dark text-white" : "bg-neutral-100 dark:bg-neutral-800")}>{d}</button>
              ))}
            </div>
          )}
          {Number.isFinite(whenUnix) && (
            <p className="text-xs text-neutral-500">
              {repeat === "once" ? (inPast ? "" : `Will send ${fmt(whenUnix)}.`) : `Next run ${fmt(whenUnix > Date.now() / 1000 ? whenUnix : nextOccurrence({ next_run: whenUnix, repeat, weekdays: weekdays.join(",") }, Math.floor(Date.now() / 1000)) ?? whenUnix)}, then ${repeat}.`}
            </p>
          )}
        ...
        <div className="flex justify-end gap-2 p-3 border-t border-neutral-200 dark:border-neutral-800">
          ...
          <Button disabled={!valid || busy} onClick={save}>{busy ? <Loader2 size={14} className="animate-spin" /> : initial ? "Save" : "Schedule"}</Button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="w-[480px] max-h-[70vh] flex flex-col rounded-xl bg-white dark:bg-neutral-900 shadow-2xl">
        <div className="flex items-center gap-2 p-3 border-b border-neutral-200 dark:border-neutral-800">
          ...
          <span className="font-semibold flex-1 truncate">History · {schedule.target_type === "status" ? "My status" : schedule.target_name || schedule.target_id}</span>
          <button onClick={onClose}><X size={16} /></button>
        </div>
        <ul className="flex-1 overflow-y-auto divide-y divide-neutral-100 dark:divide-neutral-800 text-sm">
          {q.isLoading && <li className="p-4"><Loader2 className="animate-spin text-neutral-400" /></li>}
          ...
          {q.data?.map((r) => (
            <li key={r.id} className="flex items-start gap-3 px-4 py-2">
              {r.status === "ok" ? <CheckCircle2 size={16} className="text-emerald-600 mt-0.5" /> : <AlertTriangle size={16} className={cn("mt-0.5", r.status === "missed" ? "text-amber-600" : "text-red-600")} />}
              <span className="min-w-0 flex-1">
                <span className="block">{fmt(r.ran_at)} · <span className="capitalize">{r.status}</span></span>
