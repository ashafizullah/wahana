    ...
    <div className="inline-flex items-center gap-1">
      <Popover
        ...
        trigger={
          <Button
            ...
            variant="secondary"
            disabled={!ready || busy}
            title={ready ? "Write this message from a short brief" : "Set up AI in Settings first"}
            onClick={() => setOpen((v) => !v)}
          >
      ...
      >
        <div className="text-xs text-neutral-500">
          What should the message say? Facts only — the AI writes the wording in your persona's voice.
        </div>
        <textarea
          autoFocus
          value={brief}
          ...
          placeholder={
            kind === "status"
              ? "e.g. closed tomorrow for Idul Adha, open again Thursday"
              : "e.g. 20% off all cables this weekend, pickup only, mention free parking"
          }
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button size="sm" disabled={!brief.trim() || busy} onClick={() => void run()}>
      ...
      {undo !== null && (
        <Button
          size="sm"
          variant="ghost"
          title="Restore the previous text"
          onClick={() => {
            onResult(undo);
            setUndo(null);
          }}
