...
export function LabelsDialog({
  ...
  const suggest = async () => {
    ...
    try {
      ...
      setAi(
        await suggestLabels(transcript(usable, resolveName), {
          chatName,
        ...
        <div className="p-4 space-y-3">
            ...
            {(labels ?? []).map((l) => (
              <li key={l.id} className="flex items-center gap-2">
                ...
                <button
                  ...
                  title="Delete label"
                  onClick={async () => {
                    if (await confirm({ title: `Delete label "${l.name}"?`, danger: true, confirmLabel: "Delete" })) {
                      await requireClient()
                        .deleteLabel(session, l.id)
          ...
          <div className="flex items-center gap-2">
              ...
              {LABEL_COLORS.slice(0, 5).map((c) => (
                <button
                  ...
                  className={cn(
                    ...
                    newColor === c ? "border-neutral-800 dark:border-white" : "border-transparent",
                  )}
                  style={{ background: c }}
                />
              ))}
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  await requireClient().createLabel(session, newName.trim(), newColor);
                  setNewName("");
                  invalidate();
                } catch (e) {
                  setErr(errMsg(e));
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-1">
                      ...
                      {ai.labels.map((name) => {
                        ...
                        return (
                          <button
                            key={name}
                            onClick={() => applySuggested([name])}
                            disabled={on}
                            className={cn(
                              "inline-flex items-center gap-1 rounded-full border px-2 py-0.5",
                              on
                                ? "border-wa-dark text-wa-dark"
                                : "border-neutral-300 dark:border-neutral-600 hover:bg-white dark:hover:bg-neutral-700",
                            )}
                          >
                            <span className="w-2 h-2 rounded-full" style={{ background: l?.colorHex || "#999" }} />
                            {name}
                            {on && <Check size={10} />}
                          </button>
                        );
                      ...
                      {ai.labels.some((name) => {
                        const l = labels?.find((x) => x.name === name);
                        return l && !sel.has(l.id);
                      }) && (
                        <button className="underline text-wa-dark" onClick={() => applySuggested(ai.labels)}>
                          apply all
                        </button>
                      )}
                    </div>
                  ...
                  {ai.suggestNew && (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        className="rounded-full border border-dashed border-neutral-400 px-2 py-0.5 hover:bg-white dark:hover:bg-neutral-700"
                        onClick={() => {
                          setNewName(ai.suggestNew!);
                        }}
                      >
                        + {ai.suggestNew}
                      </button>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                setBusy(true);
                try {
                  await requireClient().setChatLabels(session, chatId, [...sel]);
                  invalidate();
                  onClose();
                } catch (e) {
                  setErr(errMsg(e));
